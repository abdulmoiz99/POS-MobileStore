...
{
    public partial class frm_UnitSetup : Form
    {
        int mode;
        string FName = "UnitSetup";
        public frm_UnitSetup()
        {
        ...
        private void frm_UnitSetup_Load(object sender, EventArgs e)
        {
            Main.fillDgv(dgv_BrandSetup, "select UID,UName,Uactive from UnitSetup where CompanyID=" + Main.CompanyID + "");
        }
        ...
        {
            Main.fillDgv(dgv_BrandSetup, "select UID,UName,UActive from UnitSetup where CompanyID=" + Main.CompanyID + " AND CAST(UName as varchar) like '%" + txt_Find.Text + "%'");
        }
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            ...
            if (mode == 1)
            {
                ...
                else
                {
                    ...
                    {
                        SQL.NonScalarQuery("Insert Into UnitSetup(UName,UActive,CompanyID) values ('" + txt_Name.Text + "'," + active + "," + Main.CompanyID + ")");
                        AllClear();
                        frm_UnitSetup_Load(sender, e);
            ...
            else if (mode == 2)
            {
                ...
                else
                {
                    ...
                    if (YesOrNo == DialogResult.Yes)
                    {
                        SQL.NonScalarQuery("update UnitSetup set UName='" + txt_Name.Text + "',UActive=" + active + " where UID=" + int.Parse(txt_DataGridViewIndex.Text) + "");
                        AllClear();
                        frm_UnitSetup_Load(sender, e);
                        btn_cancel_Click(sender, e);
