...
{
    public partial class frm_ColorSetup : Form
    {
        int mode;
        string FName="ColorSetup";
        public frm_ColorSetup()
        {
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            ...
            if (mode == 1)
            {
                ...
                else
                {
                    ...
                    {
                        SQL.NonScalarQuery("Insert Into ItemColor(ICName,ICActive,CompanyID) values ('" + txt_Name.Text + "'," + active + "," + Main.CompanyID + ")");
                        AllClear();
                        frm_colorSetup_Load(sender, e);
            ...
            else if (mode == 2)
            {
                ...
                else
                {
                    ...
                    if (YesOrNo == DialogResult.Yes)
                    {
                        SQL.NonScalarQuery("update ItemColor set ICName='" + txt_Name.Text + "' ,ICActive =" + active + "where ICID=" + int.Parse(txt_DataGridViewIndex.Text) + "");
                        AllClear();
                        frm_colorSetup_Load(sender, e);
                        btn_cancel_Click(sender, e);
        ...
        {
            Main.fillDgv(dgv_BrandSetup, "select ICID,ICName,ICActive from ItemColor where CompanyID=" + Main.CompanyID + " AND CAST(ICName as varchar) like '%" + txt_Find.Text + "%'");
        }
        private void frm_colorSetup_Load(object sender, EventArgs e)
        {
            Main.fillDgv(dgv_BrandSetup, "select ICID,ICName,ICactive from ItemColor where CompanyID=" + Main.CompanyID + "");
        }
