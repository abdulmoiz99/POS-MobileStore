...
{
    public partial class frm_TypeSetup : Form
    {
        int mode;
        String FName = "TypeSetup";
        public frm_TypeSetup()
        {
        ...
        private void frm_TypeSetup_Load(object sender, EventArgs e)
        {
            Main.fillDgv(dgv_BrandSetup, "select ITID,ITName,ITactive from ItemType where CompanyID=" + Main.CompanyID + "");
        }
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            ...
            if (mode == 1)
            {
                ...
                else
                {
                    ...
                    {
                        SQL.NonScalarQuery("Insert Into ItemType(ITName,ITActive,CompanyID) values ('" + txt_Name.Text + "'," + active + "," + Main.CompanyID + ")");
                        AllClear();
                        frm_TypeSetup_Load(sender, e);
            ...
            else if (mode == 2)
            {
                ...
                else
                {
                    ...
                    if (YesOrNo == DialogResult.Yes)
                    {
                        SQL.NonScalarQuery("update ItemType set ITName='" + txt_Name.Text + "' , ITActive=" + active + "where ITID=" + int.Parse(txt_DataGridViewIndex.Text) + "");
                        AllClear();
                        frm_TypeSetup_Load(sender, e);
                        btn_cancel_Click(sender, e);
        ...
        {
            Main.fillDgv(dgv_BrandSetup, "select ITID,ITName,ITActive from ItemType where CompanyID=" + Main.CompanyID + " AND CAST(ITName as varchar) like '%" + txt_Find.Text + "%'");
        }
